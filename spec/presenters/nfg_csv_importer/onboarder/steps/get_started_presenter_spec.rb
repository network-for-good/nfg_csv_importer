...
describe NfgCsvImporter::Onboarder::Steps::GetStartedPresenter do
  ...
  let(:get_started_step_presenter) { described_class.new(onboarding_session, h) }
  ...
  describe '#get_started_information_step(step:)' do
    let(:step) { 1 }
    subject { get_started_step_presenter.get_started_information_step(step: step) }
  ...
  describe '#get_started_information_step_image(step:)' do
    ...
    subject { get_started_step_presenter.get_started_information_step_image(step: step) }
  ...
  describe '#show_time_estimate?' do
    subject { get_started_step_presenter.show_time_estimate? }
    ...
    context 'when the import file_origination_type is not a third-party' do
      ...
      before { get_started_step_presenter.stubs(:file_origination_type).returns(file_origination_type) }
  ...
  describe '#external_resource_url' do
    subject { get_started_step_presenter.external_resource_url }
