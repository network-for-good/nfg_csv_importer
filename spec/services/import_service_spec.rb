...
describe NfgCsvImporter::ImportService do
  ...
  describe "#rows_with_required_columns" do
    ...
    context 'when MAX_ROWS_TO_SCAN have been scanned' do
      let(:file_name) { "/import.csv" }
      let(:first_row) { {"amount"=>"10", "donated_at"=>"2/2/19", "email"=>"pathakh@msn.com", "first_name"=>nil, "last_name"=>'man', "payment_method"=>"credit_card"} }
      let(:max_rows_to_scan) { 3 }

      before { NfgCsvImporter::ImportService::MAX_ROWS_TO_SCAN = max_rows_to_scan }
  ...
  describe "#populate_statistics" do
    ...
    before { NfgCsvImporter::PreviewTemplateService.any_instance.stubs(:nfg_csv_importer_to_host_mapping).returns(nfg_csv_importer_to_host_mapping) }
