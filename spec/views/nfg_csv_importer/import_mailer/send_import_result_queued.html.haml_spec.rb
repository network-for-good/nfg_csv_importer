...
RSpec.describe "nfg_csv_importer/import_mailer/send_import_result_queued.html.haml", type: :view do
  ...
  subject { render template: 'nfg_csv_importer/import_mailer/send_import_result_queued.html.haml' }

  it 'renders step template' do
    expect(subject).to render_template 'nfg_csv_importer/import_mailer/send_import_result/_steps'
  end

  it 'renders the right content pertaining to queued status' do
    expect(subject).to have_content I18n.t('mailers.nfg_csv_importer.send_import_result_mailer.body.introduction.queued')
  end
end
